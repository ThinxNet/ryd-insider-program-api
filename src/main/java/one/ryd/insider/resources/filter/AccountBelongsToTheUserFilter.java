...
public class AccountBelongsToTheUserFilter implements ContainerRequestFilter {
  ...
  public void filter(final ContainerRequestContext ctx) throws IOException {
    ...
    final List<ObjectId> userIds = this.userIds(
      new ObjectId(accountIdStr), AccountRole.ACCOUNT_OWNER
    );
    if (Objects.isNull(userIds) || !userIds.contains(entity.entity().getId())) {
      ctx.abortWith(Response.status(Response.Status.NOT_FOUND).build());
      return;
    }
  }

  // @todo #7 make role configurable via the annotation
  private List<ObjectId> userIds(final ObjectId accountId, final AccountRole role) {
    final Account account = this.dsInsider.get(Account.class, accountId);
    return Objects.isNull(account) ? null : account.getUsers().stream()
